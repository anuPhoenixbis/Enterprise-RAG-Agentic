...
def parse_pdf(file_path: str)-> str:
    ...
    with logfire.span("PDF Parsing (local)", file_path):
        try:
            ...
            for i, page in enumerate(reader.pages):
                text = page.extract_text() or "" #extract the text or else give empty str
                if text.strip():
                    text_parts.append(text) # collect the texts
                else:
                    blank_pages.append(i+1) # get the blank pages' indices to be skipped
                ...
                try:
                    ...
                    with pdfplumber.open(file_path) as pdf:
                        for page_num in blank_pages:
                            page = pdf.pages[page_num-1] # trying to extract the blank pages again
                            fallback_text = page.extract_text() or  ""
                            if fallback_text.strip():
                                text_parts.append(fallback_text)#append the more retrieved pages
                except Exception as err:
                    logfire.warning(f"pdfplumber fallback failed: {err}")
